...
When the user presses the Encrypt button, the program reads the plaintext from the input area, converts it to UTF-8 bytes, and encrypts it. The encrypted bytes are then converted into Base64 text. I decided to show the result in Base64 instead of raw binary because it is much more readable in the user interface and in the screenshots. When the Decrypt button is pressed, the program reads the Base64 ciphertext, decodes it back into bytes, and decrypts it with the same AES key.
...
I also added a small command-line check class called \texttt{AesRoundTripCheck}. This file is useful because it verifies the same encryption and decryption logic without opening the GUI. In addition, the GUI class can generate screenshots automatically. This made it easier to create clean figures for the report and to repeat the process if needed.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/gui-launch.png}
    \caption{Program window at startup with the required plaintext already loaded.}
    \label{fig:launch}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/gui-encrypted.png}
    \caption{The GUI after encryption, showing the generated AES key and the ciphertext in Base64 form.}
    \label{fig:encrypted}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/gui-decrypted.png}
    \caption{The GUI after decryption, where the original plaintext has been restored.}
    \label{fig:decrypted}
\end{figure}
